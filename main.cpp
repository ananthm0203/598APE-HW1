...
Autonoma* createInputs(const char* inputFile) {
    ...
    Autonoma* MAIN_DATA =
        new Autonoma(Camera(Vector(camera_x, camera_y, camera_z), yaw, pitch, roll), background);
    ...
    if (f) {
        ...
        while (lscanf(f, "%s", object_type) != EOF) {
            if (streq(object_type, "light")) {
                ...
                Plane shape(Vector(plane_x, plane_y, plane_z), texture, yaw, pitch, roll, tx, ty);
                shape.normalMap = parseTexture(f, true);
                MAIN_DATA->addPlane(std::move(shape));
                ...
                Texture* texture = parseTexture(f, false);
                Disk     shape(Vector(disk_x, disk_y, disk_z), texture, yaw, pitch, roll, tx, ty);
                shape.normalMap = parseTexture(f, true);
                MAIN_DATA->addDisk(std::move(shape));
                ...
                Texture* texture = parseTexture(f, false);
                Box      shape(Vector(box_x, box_y, box_z), texture, yaw, pitch, roll, tx, ty);
                shape.normalMap = parseTexture(f, true);
                MAIN_DATA->addBox(std::move(shape));
                ...
                Texture* texture = parseTexture(f, false);
                Triangle shape(Vector(x1, y1, z1), Vector(x2, y2, z2), Vector(x3, y3, z3), texture);
                shape.normalMap = parseTexture(f, true);
                MAIN_DATA->addTriangle(std::move(shape));
                ...
                Texture* texture = parseTexture(f, false);
                Sphere   shape(Vector(sphere_x, sphere_y, sphere_z), texture, yaw, pitch, roll,
                             radius);
                shape.normalMap = parseTexture(f, true);
                MAIN_DATA->addSphere(std::move(shape));
                ...
                if (lscanf(f, "%s %d %s %d %lf %lf %lf\n", point_filepath, &num_points,
                    ...
                    printf("Could not read <point filepath> <num_points> <polygons "
                           "filepath> "
                           "<num_polygons> <off_x> <off_y> <off_z>\n");
                    exit(1);
                }
                Texture* texture   = parseTexture(f, false);
                ...
                for (int i = 0; i < num_polygons; i++) {
                    Triangle shape(points[polys[3 * i]] + offset, points[polys[3 * i + 1]] + offset,
        ...
        while (lscanf(f, "%s %s %d %s %lf %lf", transition_type, object_type, &obj_num, field_type,
            ...
            if (streq(object_type, "camera")) {
                ...
                if (streq(field_type, "yaw")) {
                    ...
                    printf("Unknown shape field_type %s, expected one of yaw, pitch, roll, "
                           "textureX, textureY, mapX, mapY, mapOffX, mapOffY\n",
                           field_type);
                    exit(1);
                }
            } else {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (streq(argv[i], "--help")) {
            printf("Usage %s [-H <height>] [-W <width>] [-F <framecount>] [--movie] "
                   "[--no-movie] "
                   "[--png] [--ppm] [--help] [-o <outfile>] [-i <infile>]\n",
                   argv[0]);
            return 0;
        }
    ...
    if (frameLen > 1 && toMovie) {
        if (png) {
            snprintf(command, sizeof(command),
                     ...
                     "ffmpeg -y -i "
                     "%s.tmp.avi -c:v libx264 -preset veryslow -qp 0 -r 24 %s",

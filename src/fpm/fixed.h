...
namespace fpm {
    ...
    //! \tparam BaseType         the base integer type used to store the fixed-point number. This
    //! can be a signed or unsigned type. \tparam IntermediateType the integer type used to store
    //! intermediate results during calculations. \tparam FractionBits     the number of bits of the
    //! BaseType used to store the fraction \tparam EnableRounding   enable rounding of LSB for
    //! multiplication, division, and type conversion
    ...
    class fixed {
        ...
        static_assert(FractionBits <= sizeof(BaseType) * 8 - 1,
                      "BaseType must at least be able to contain entire fraction, with space for "
                      "at least one integral bit");
        static_assert(sizeof(IntermediateType) > sizeof(BaseType),
        ...
        // Although this value fits in the BaseType in terms of bits, if there's only one integral
        // bit, this value is incorrect (flips from positive to negative), so we must extend the
        // size to IntermediateType.
    ...
    public:
        ...
        constexpr inline explicit fixed(T val) noexcept
            : m_value(static_cast<BaseType>((EnableRounding) ? (val >= 0.0)
                                                                   ? (val * FRACTION_MULT + T{0.5})
                                                                   : (val * FRACTION_MULT - T{0.5})
                                                             : (val * FRACTION_MULT))) {}

        // Constructs from another fixed-point type with possibly different underlying
        // representation. Like static_cast, this truncates bits that don't fit.
        ...
        constexpr inline BaseType raw_value() const noexcept { return m_value; }
        ...
        static constexpr inline fixed from_fixed_point(T value) noexcept {
            ...
            // We do this by multiplying by two before dividing and adding the LSB to the real
            // result.
            return (EnableRounding)
                       ? fixed(static_cast<BaseType>(
                                   value / (T(1) << (NumFractionBits - FractionBits)) +
                                   (value / (T(1) << (NumFractionBits - FractionBits - 1)) % 2)),
                               raw_construct_tag{})
                       : fixed(static_cast<BaseType>(value /
                                                     (T(1) << (NumFractionBits - FractionBits))),
                               raw_construct_tag{});
        }
        ...
        template <unsigned int NumFractionBits, typename T,
                  typename std::enable_if<(NumFractionBits <= FractionBits)>::type* = nullptr>
        static constexpr inline fixed from_fixed_point(T value) noexcept {
            return fixed(static_cast<BaseType>(value * (T(1) << (FractionBits - NumFractionBits))),
                         raw_construct_tag{});
        }
        ...
        inline fixed& operator*=(const fixed& y) noexcept {
            if (EnableRounding) {
                ...
                // To correctly round the last bit in the result, we need one more bit of
                // information. We do this by multiplying by two before dividing and adding the LSB
                // to the real result.
                auto value =
                    (static_cast<IntermediateType>(m_value) * y.m_value) / (FRACTION_MULT / 2);
        ...
        inline fixed& operator/=(const fixed& y) noexcept {
            ...
            if (EnableRounding) {
                ...
                // to the real result.
                auto value =
                    (static_cast<IntermediateType>(m_value) * FRACTION_MULT * 2) / y.m_value;
                m_value = static_cast<BaseType>((value / 2) + (value % 2));
            } else {
    ...
    template <typename B, typename I, unsigned int F, bool R>
    constexpr inline fixed<B, I, F, R> operator+(const fixed<B, I, F, R>& x,
                                                 const fixed<B, I, F, R>& y) noexcept {
    ...
    template <typename B, typename I, unsigned int F, bool R>
    constexpr inline bool operator==(const fixed<B, I, F, R>& x,
                                     const fixed<B, I, F, R>& y) noexcept {
...
namespace std {
    ...
    template <typename B, typename I, unsigned int F, bool R> struct hash<fpm::fixed<B, I, F, R>> {
    ...
    struct numeric_limits<fpm::fixed<B, I, F, R>> {
        ...
        static constexpr int max_digits10 =
            fpm::detail::max_digits10(std::numeric_limits<B>::digits - F) +
        ...
        static constexpr int max_exponent10 =
            fpm::detail::digits10(std::numeric_limits<B>::digits - F);
        ...
        static constexpr fpm::fixed<B, I, F, R> min() noexcept { return lowest(); }
        ...
        static constexpr fpm::fixed<B, I, F, R> denorm_min() noexcept { return min(); }
    };
...
namespace fpm {
    ...
    template <typename BaseType, typename IntermediateType, unsigned int FractionBits,
              bool EnableRounding>
    struct is_fixed<fixed<BaseType, IntermediateType, FractionBits, EnableRounding>>
        : std::true_type {};

#if __cplusplus >= 201703L
    template <typename T> inline constexpr bool is_fixed_v = is_fixed<T>::value;
#endif
} // namespace fpm
#endif